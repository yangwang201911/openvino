        ...
        if (u->get_node().is_type<crop>()) {
            if (u->get_node().can_be_optimized()) {
                ...
                if (!crop_users.empty() && crop_users.front()->get_node().is_type<reshape>()) {
                    ...
                    // Refresh the reshape output shape after the crop predecessor has been resolved
                    // above, so the reshape resolves its dynamic dimensions (e.g. batch) instead of
                    // staying dynamic. user_info is populated unconditionally so the in-place padding
                    // propagation below can correct the reshape output layout regardless of whether the
                    // main flow already updated its shape. The main shape-update flow may have
                    // run reshape->update_shape() earlier, while the crop predecessor was still
                    // dynamic, leaving the reshape output shape unresolved and marking
                    // _update_shape_done_by_other. Reset that flag so update_shape() actually
                    // recomputes from the now-resolved crop output instead of early-returning.
                    reshape_inst->_update_shape_done_by_other = false;
                ...
                // along_feature path is used only for crops without a reshape user.
                if (crop_in_place_optimization::can_crop_be_optimized_along_feature(crop_layout, pred_layout) && !user_info.first) {
                    // Feature-axis in-place crop with no reshape user: pad the crop output directly.
