...
// -----------------------------------------------------------------------------------
// ov::intel_auto::low_power_device must take precedence over both perf_curve_table and
// devices_utilization_threshold whenever the platform is reported as being in low power mode.
// get_low_power_mode() is mocked directly since the real IPF/DTT probe is only a placeholder.
// -----------------------------------------------------------------------------------
...
protected:
    std::string netPrecision = "FP32";
    std::vector<DeviceInformation> devices = {{"CPU", {}, -1, "01", "CPU_01", 0}, {"NPU", {}, -1, "01", "NPU_01", 0}};
    // NPU exceeds this threshold and loses perf_curve_table scoring; only low_power_device can pick it.
...
TEST_F(SelectDeviceWithLowPowerDevicePrecedenceTest, lowPowerDeviceOverridesPerfCurveTableAndThreshold) {
    EXPECT_CALL(*plugin, get_low_power_mode()).WillOnce(Return(true));
    auto result = plugin->select_device(devices, netPrecision, 0, thresholds, perfCurveTable, "NPU");
    selectedUniqueName = result.unique_name;
    EXPECT_EQ(result.unique_name, "NPU_01");
}

TEST_F(SelectDeviceWithLowPowerDevicePrecedenceTest, fallsBackToPerfCurveTableWhenNotInLowPowerMode) {
    EXPECT_CALL(*plugin, get_low_power_mode()).WillOnce(Return(false));
    auto result = plugin->select_device(devices, netPrecision, 0, thresholds, perfCurveTable, "NPU");
    selectedUniqueName = result.unique_name;
    // perf_curve_table picks CPU (lower score) once low_power_device is not applicable.
    EXPECT_EQ(result.unique_name, "CPU_01");
}
...
TEST_F(SelectDeviceWithLowPowerDevicePrecedenceTest, lowPowerDeviceNotInCandidateListFallsThrough) {
    EXPECT_CALL(*plugin, get_low_power_mode()).WillOnce(Return(true));
    auto result = plugin->select_device(devices, netPrecision, 0, thresholds, perfCurveTable, "GPU.0");
    selectedUniqueName = result.unique_name;
    EXPECT_EQ(result.unique_name, "CPU_01");
}
