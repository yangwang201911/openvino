...
int tmain(int argc, tchar* argv[]) {
    try {
        ...
        if (argc != 5) {
            slog::info << "Usage : " << argv[0] << " <path_to_model> <path_to_image> <device_name> <loop_times>" << slog::endl;
            return EXIT_FAILURE;
        }
        ...
        int loop_times = std::atoi(argv[4]);  
        ...
        for (int index = 0; index < loop_times; index++) {
            ...
            std::vector<float> m_detectionTresholds = {0.5,0.5};
            ...
            for (int i = 0; i < maxProposalCount; i++) {
                ...
                x = static_cast<int>(detections[i * objectSize + 3] * reader->width());
                if(x < 0)
                    x = 0;
                y = static_cast<int>(detections[i * objectSize + 4] * reader->height());
                if(y < 0)
                    y = 0;
                width = static_cast<int>(detections[i * objectSize + 5] * reader->width()) - x;
                if(width > reader->width())
                    width = reader->width();
                height = static_cast<int>(detections[i * objectSize + 6] * reader->height()) - y;
                if(height > reader->height())
                    height = reader->height();
                std::cout << i << "," << label << "," << confidence
                            << "," << x << "," << y << "," << width << "," << height << ",";
            }
            std::cout << "\n";
            // -----------------------------------------------------------------------------------------------------
