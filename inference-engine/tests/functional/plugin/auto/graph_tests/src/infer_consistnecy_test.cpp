...
std::vector<InferenceEngine::Blob::Ptr> AutoInferConsistency::getBaseCPUOutputs() {
...
void AutoInferConsistency::loadBaseCPUNet() {
    _baseExecNet = core->LoadNetwork(cnnNetwork, _baseDevice, _baseConfig);
}

void AutoInferConsistency::inferBaseCpu() {
...
TEST_P(AutoInferConsistency, CompareWithDirectHW) {
    ...
    LoadNetwork();
    //Load function to CPU base device
    loadBaseCPUNet();
    for (size_t i = 0; i < getInferCount(); i++) {
        ...
        const auto &expectedOutputs = getBaseCPUOutputs();
